...
def derisk(gas):
    ...
    if matic_balance >= 50:
        amountOutMin = w3.toWei((matic_balance / 2) * .99 * matic_price, 'ether')
        ...
        swap = quick_router.functions.swapExactETHForTokens(amountOutMin, path, myAddress, deadline).buildTransaction({'from': myAddress,
                                                                                                                        ...
                                                                                                                        'gas': 2000000,
                                                                                                                        'gasPrice': w3.toWei(gas, 'gwei'),
                                                                                                                        'value': w3.toWei(matic_balance / 2, 'ether')
                                                                                                                            })
...
def liqLoop(nonce):
    ...
    for v in vaults_dict:
        if vaults_dict[v][0] * matic_price <= 1.4925 * vaults_dict[v][1] and vaults_dict[v][0] * matic_price > 1.40 * vaults_dict[v][1] and balance > vaults_dict[v][1]/2:
            try:
...
while True:
    try:
        if random.randint(1, 150000) == 10 or len(vaults) < 10:
            slowVaultCheck()

        elif random.randint(1, 10000) == 69:
            fastVaultCheck()
